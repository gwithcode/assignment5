...
def read_all_resource(db: Session):
    return db.query(models.Resource).all()

def read_one_resource(db: Session, resource_id):
    return db.query(models.Resource).filter(models.Resource.id == resource_id)

def update_one_resource(db: Session, resource_id, resource):
    db_resource = db.query(models.Resource).filter(models.Resource.id == resource_id)

    update_resource = resource.model_dump(exclude_unset = True)

    db_resource.update(update_resource, synchronize_session = False)

    db.commit()

    return db_resource.first()

def delete_one_resource(db: Session, resource_id):
    db_resource = db.query(models.Resource).filter(models.Resource.id == resource_id)
